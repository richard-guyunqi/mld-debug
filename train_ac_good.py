...
parser.add_argument('--metadata_path', default='s3://pixai-test-uw2/richard/final_Danbooru_5.parquet', type=str)
...
#             self.model.load_state_dict(torch.load(self.args.default_ckpt))
#             print('Success')
#         if self.args.ema_ckpt:
#             self.ema.load_state_dict(torch.load(self.args.ema_ckpt))
        
    
        # To check that model save path is a valid dir
        logger.info(f'Check ckpt save path: {os.path.isdir(self.args.out_dir)}')
    ...
    def initialize_train_val_split(self, transform):
        ...
        total_size = len(df)
        train_size = int(total_size * 0.9)  # 80% for training
    ...
    def build_scheduler(self):
        self.scheduler = lr_scheduler.OneCycleLR(self.optimizer, max_lr=self.args.max_lr,
                                                 steps_per_epoch=self.steps_per_epoch, epochs=self.args.epochs, pct_start=0.1)
        ...
        if self.local_rank == 0:
            try:
                torch.save({'model': self.model.module.state_dict(), 'ema': self.ema.module.state_dict()}, os.path.join(
                    self.args.out_dir, f'{model_name}-{self.epoch + 1}-{step + 1}.ckpt'))
                logger.info(f'Model ckpt saved: {self.epoch + 1}-end, {self.args.out_dir}/{model_name}-{self.epoch + 1}-{step + 1}')
                
            except:
                logger.info(f'CKPT SAVE UNSUCCESSFUL {self.epoch + 1}-end, {self.args.out_dir}/{model_name}-{self.epoch + 1}-{step + 1}')
